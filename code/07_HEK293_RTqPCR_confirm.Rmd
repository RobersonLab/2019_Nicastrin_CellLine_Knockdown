---
title: "HEK293 Knockdown confirm RT-qPCR"
author: "E. Roberson"
date: "March 5, 2018"
...
## Import data
```{r read_data}
rtqpcr <- read_csv( knockdown_hek293_rtqpcr_confirm, col_names=TRUE ) %>%
	select( -Batch ) %>%
	mutate( Construct = str_replace( Construct, "pLKO", "shLuc" ) ) %>%
	mutate( Construct = factor( Construct, levels=c( "shLuc", "A08" ) ) ) %>%
	mutate( Tech_Rep = factor( Tech_Rep, levels=c( "i", "ii", "iii" ) ) ) %>%
	mutate( Amplicon = str_replace( Amplicon, "NCSTN_Amp2", "NCSTN" ) ) %>%
...
dCq <- ddply( .data = collapse_tech_reps, .variables = c( "Construct" ), .fun = deltaCqs )
...
```{r hek293_shRNA_qPCR_confirmation}
up.qpcr.plot <- filter( ddCq, foldchange_mean > 1.0 ) %>%
  ...
  geom_abline( intercept=1.0, slope=0, lty=2 ) +
	...
	ggtitle( "HEK293 RT-qPCR - increased expression\n" ) +
	gg_center_title +
...
down.qpcr.plot <- filter( ddCq, foldchange_mean < 1.0 ) %>%
  ...
  xlab( "\nAmplicon" ) +
  ylab( "Fold-change\n" ) +
	scale_y_continuous( limits = c( 0.0, 1.4 ), breaks=seq( 0.0, 1.4, 0.2 ) ) +
  theme_bw() +
  scale_colour_manual( values=cbPalette ) +
  gg_no_grid +
	theme( legend.position="top" ) +
  ...
  geom_abline( intercept=1.0, slope=0, lty=2 ) +
	...
	geom_point( size=3, position=position_dodge(0.3) ) +
	ggtitle( "HEK293 RT-qPCR - decreased expression\n" ) +
	gg_center_title +
	theme( axis.text = element_text( size=18 ) ) +
...
```{r hek293_rtqpcr_confirm_combined}
